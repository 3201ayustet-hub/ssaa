...
const KEY='travel-territory-state-v1.2';
const PREFS=[
...
const DEFAULT={
  game:{period:1,periodPoints:{p1:0,p2:0,p3:0,p4:0},cumulativePoints:{p1:0,p2:0,p3:0,p4:0},residencePoints:5,otherPoints:10,lastPointMonth:''},
  players:[
    {id:'p1',name:'プレイヤー1',residence:'',color:'p1'},
    {id:'p2',name:'プレイヤー2',residence:'',color:'p2'},
    {id:'p3',name:'プレイヤー3',residence:'',color:'p3'},
    {id:'p4',name:'プレイヤー4',color:'p4',residence:''}
  ],
  stays:[], settlements:[]
};
let state=load();
let route='map', selectedPref=null, selectedPlayer=null, photoData='', photoName='';
...
function load(){
  try{
    ...
    return {
      ...
      players:Array.isArray(x.players)?x.players:b.players,
...
function pointsForStay(pid,code){
  ...
  const region=Pref(code).region;
  const homeRegion=Pref(p?.residence)?.region;
  return region&&homeRegion&&region===homeRegion?state.game.residencePoints:state.game.otherPoints;
}
...
function recalcPoints(){
  ...
  for(const p of state.players)next[p.id]=(next[p.id]||0);
  for(const [code,v] of Object.entries(o)){
    ...
    state.game.cumulativePoints[v.playerId]=(state.game.cumulativePoints[v.playerId]||0)+base*mult;
  }
...
function render(){
  recalcPoints();
  document.querySelector('#app').innerHTML=`<main class="app">${route==='map'?mapScreen():route==='record'?recordScreen():route==='history'?historyScreen():route==='setup'?setupScreen():route==='admin'?adminScreen():menuScreen()}</main>${nav()}`;
  if(route==='map')loadMap();
}
function nav(){
  return `<nav class="bottom-nav">
    ${[['map','▦','マップ'],['record','✦','記録'],['history','▤','履歴'],['menu','☰','メニュー']].map(([r,i,t])=>`<button class="nav-btn ${route===r?'active':''}" data-route="${r}"><span>${i}</span>${t}</button>`).join('')}
  </nav>`;
}
function mapScreen(){
  const completed=state.players.flatMap(p=>completedRegions(p.id).map(r=>({p,r})));
  return `<header class="topbar"><div><div class="eyebrow">TRAVEL TERRITORY</div><h1>日本全国陣取り</h1><div class="period">第${state.game.period}期</div></div></header>
  <section class="map-wrap"><div id="map">地図を読み込み中…</div><div class="hokkaido-route" aria-hidden="true"></div></section>
...
function styleMap(svg){
  ...
  // 地方の境界線は地図上には描かない。地方データ・制覇ロジックは残す。
}
...
function recordScreen(){
  ...
  <form class="form" id="record-form">
    <div class="field"><label for="record-player">プレイヤー</label><select id="record-player" required>${state.players.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('')}</select></div>
    ...
    <div class="field"><label>証拠写真 1枚</label><label class="file-picker"><span>写真を選択</span><span id="photo-name">${esc(photoName||'未選択')}</span><input id="record-photo" type="file" accept="image/*" capture="environment" required></label><div id="photo-preview"></div></div>
    <div class="field"><label for="record-comment">コメント（任意）</label><textarea id="record-comment" maxlength="300"></textarea></div>
    <button class="primary" id="confirm-record" type="button" disabled>登録の確認画面へ</button>
  </form>`;
}
function bindRecord(){
  const form=document.querySelector('#record-form'); if(!form)return;
  const photo=document.querySelector('#record-photo'), btn=document.querySelector('#confirm-record'), name=document.querySelector('#photo-name'), preview=document.querySelector('#photo-preview');
  const update=()=>{btn.disabled=!(form.querySelector('#record-player').value&&form.querySelector('#record-pref').value&&form.querySelector('#record-date').value&&form.querySelector('#record-type').value&&photo.files?.length&&photoData)};
  photo.addEventListener('change',()=>{
    const file=photo.files?.[0]; if(!file){photoData='';photoName='';name.textContent='未選択';preview.innerHTML='';update();return}
    photoName=file.name; name.textContent=file.name;
    const reader=new FileReader();
    reader.onload=()=>{photoData=reader.result;preview.innerHTML=`<img class="preview" src="${photoData}" alt="証拠写真プレビュー">`;update()};
    reader.readAsDataURL(file);
  });
  form.querySelectorAll('input,select,textarea').forEach(x=>x.addEventListener('input',update));
  btn.addEventListener('click',()=>{
    if(btn.disabled)return;
    const data={playerId:document.querySelector('#record-player').value,prefecture:document.querySelector('#record-pref').value,date:document.querySelector('#record-date').value,type:document.querySelector('#record-type').value,comment:document.querySelector('#record-comment').value,photo:photoData,photoName};
    openSheet(`<button class="sheet-close" data-close>×</button><h2>登録内容の確認</h2>
      <div class="confirm-card"><div class="row"><span class="label">プレイヤー</span><b>${esc(state.players.find(p=>p.id===data.playerId)?.name)}</b></div><div class="row"><span class="label">都道府県</span><b>${Pref(data.prefecture).name}</b></div><div class="row"><span class="label">滞在日</span><b>${data.date}</b></div><div class="row"><span class="label">種別</span><b>${data.type}</b></div><img class="thumb" src="${data.photo}" alt="証拠写真"></div>
      <div class="actions"><button class="secondary" data-close>戻る</button><button class="primary" id="final-register">登録する</button></div>`);
    window.__pendingStay=data;
  });
}
function confirmPending(){
  ...
  save(); photoData='';photoName='';window.__pendingStay=null;closeSheet();route='history';render();
}
function historyScreen(){
  const rows=[...state.stays].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
  return `<h1 class="screen-title">旅行履歴</h1><p class="screen-sub">登録後の記録は内容を変更できません。テスト時は削除できます。</p>
...
function setupScreen(){
  return `<h1 class="screen-title">プレイヤー登録・変更</h1><p class="screen-sub">4人の名前・居住県・色を設定します。</p>
  <div class="setup-grid">${state.players.map((p,i)=>`<div class="player-setup"><div><strong>P${i+1}</strong><input data-p-name="${p.id}" value="${esc(p.name)}" aria-label="プレイヤー名"></div><select data-p-res="${p.id}" aria-label="居住県"><option value="">居住県なし</option>${PREFS.map(([c,n])=>`<option value="${c}" ${p.residence===c?'selected':''}>${n}</option>`).join('')}</select><select class="color-select" data-p-color="${p.id}" aria-label="プレイヤーカラー">${Object.entries(COLORS).map(([c])=>`<option value="${c}" ${p.color===c?'selected':''}>${c.toUpperCase()}</option>`).join('')}</select></div>`).join('')}</div>
  <button class="primary" id="save-players">保存する</button>`;
}
function adminScreen(){
  return `<h1 class="screen-title">管理者ページ</h1>
  <div class="admin-grid">
    ...
    <button class="danger danger-full" id="reset-all">テストデータをすべて削除</button>
  </div>`;
}
function menuScreen(){
  return `<h1 class="screen-title">メニュー</h1><div class="menu-list">
...
document.addEventListener('click',e=>{
  const routeBtn=e.target.closest('[data-route]'); if(routeBtn){route=routeBtn.dataset.route;render();return}
  if(e.target.closest('[data-close]')){closeSheet();return}
  const player=e.target.closest('[data-player]');if(player){showPlayer(player.dataset.player);return}
  const rec=e.target.closest('[data-record-pref]');if(rec){selectedPref=rec.dataset.recordPref;closeSheet();route='record';render();return}
  const del=e.target.closest('[data-delete-stay]');if(del){if(confirm('この旅行記録を削除しますか？')){state.stays=state.stays.filter(s=>s.id!==del.dataset.deleteStay);save();render()}return}
  ...
  if(e.target.id==='save-players'){
    ...
    state.players=next;save();alert('プレイヤー情報を保存しました。');render();return
  }
  if(e.target.id==='save-settings'){
    state.game.residencePoints=Number(document.querySelector('#residence-points').value)||0;
    state.game.otherPoints=Number(document.querySelector('#other-points').value)||0;save();alert('設定を保存しました。');render();return
  }
  if(e.target.id==='end-season'){
    if(!confirm(`第${state.game.period}期を終了します。ポイントを確定し、次のシーズンへ移行します。地図はそのまま残ります。`))return;
    recalcPoints();
    ...
    state.game.period++;state.game.periodPoints={p1:0,p2:0,p3:0,p4:0};state.game.lastPointMonth='';save();render();return
  }
  if(e.target.id==='reset-all'){
    if(confirm('旅行記録・ポイント・決着履歴をすべて削除します。よろしいですか？')){localStorage.removeItem(KEY);state=clone(DEFAULT);render()}return
  }
});
document.addEventListener('change',e=>{if(e.target.id==='record-photo')bindRecord()});
document.addEventListener('DOMContentLoaded',()=>{render();bindRecord()});
render();
